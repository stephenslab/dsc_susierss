#' FINEMAP I/O
write_finemap_sumstats_v1.3.1 <- function(bhat, se, allele_freq, LD_file, n, k, prefix) {
...
run_finemap_v1.3.1 <- function(bhat, se, allele_freq, LD_file, n, k, method,args = "", prefix="data")
{
  cfg = write_finemap_sumstats_v1.3.1(bhat, se, allele_freq, LD_file, n, k, prefix)
  if(method == 'sss'){
  ...
  if(method == 'sss'){
    ncausal = finemap_extract_ncausal_v1.3.1(paste0(cfg$log, '_sss'))
  }else{
    ncausal = finemap_extract_ncausal_v1.3.1(paste0(cfg$log, '_cond'))
  }

  return(list(snp=snp, config=config, set=cs, ncausal=ncausal))
}

rank_snp_v1.3.1 <- function(snp) {
...
finemap_extract_ncausal_v1.3.1 <- function(logfile)
{
...
finemap_mvar_v1.3.1 <- function(bhat, se, allele_freq, LD_file, n, k, method, args, prefix,
                         parallel = TRUE) {
  if (is.null(dim(bhat))) {
  ...
  single_core = function(r)
      run_finemap_v1.3.1(bhat[,r], se[,r], allele_freq, LD_file, n, k, method, args,
