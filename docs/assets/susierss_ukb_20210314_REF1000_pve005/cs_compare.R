...
## parameters
input = 'susierss_ukb_20210218_REF1000_pve005.2.include.purity.rds'
...
# methods = methods[-grep('susie_rss_suff_3steps_ERTRUE', methods)]
# methods = methods[-grep('susie_rss_suff_3steps_addz_ERTRUE', methods)]
# res = list()
# for(met in methods){
#   print(met)
...
rates = rates[-intersect(grep('susie_suff', rates$method),grep('ldrefout', rates$method)),]
...
## ER FALSE, AZ FALSE
rates_ERFALSE = rates[-grep('ERTRUE', rates$method),]
rates_ERFALSE = rates_ERFALSE[-grep('AZTRUE', rates_ERFALSE$method),]
rename_ERFALSE = lapply(rename, function(x) gsub('_ERFALSE','',x))

d1 = c(grep('3steps', rates_ERFALSE$method), grep('finemap', rates_ERFALSE$method)) # 3 steps and all finemap
d2 = c(setdiff(1:length(rates_ERFALSE$method), grep('3steps', rates_ERFALSE$method))) # original and all finemap

rates_ERFALSE_d1 = rates_ERFALSE[d1,]
rates_ERFALSE_d2 = rates_ERFALSE[d2,]
...
p3 = plot_panel(rates_ERFALSE_d1, c('size', 'median number of variables'), legend=F)
p4 = plot_panel(rates_ERFALSE_d1, c('purity', 'median of purity'), legend=F)
output = 'susierss_ukb_20210218_REF1000_pve005_cs/ukb_cs_3steps'
...
p3 = plot_panel(rates_ERFALSE_d2, c('size', 'median number of variables'), legend=F)
p4 = plot_panel(rates_ERFALSE_d2, c('purity', 'median of purity'), legend=F)
output = 'susierss_ukb_20210218_REF1000_pve005_cs/ukb_cs_original'
pdf(paste0(output, '_plots.pdf'), width=12, height=3)
grid.arrange(p1,p2,p3,p4, ncol=4, widths=c(3,3,3,3))
dev.off()
system(paste0("convert -flatten -density 120 ", paste0(output, '_plots.pdf'), " ", paste0(output, '_plots.png')))
