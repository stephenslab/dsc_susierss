...
for( rem in removedat_all){
  ...
  output_susierss = paste0('susierss_ukb_20210324_REF500_pve005_cs/susierss_ukb_cs_remdatall',rem)
  output_fm = paste0('susierss_ukb_20210324_REF500_pve005_cs/finemapv4_ukb_cs_remdatall',rem)
  ...
  # for(met in methods){
  #   print(met)
  #   tmp = dat$susie %>% filter(method == met)
  #   dat_sub = tmp %>% filter(!(small_data.dataset %in% nonconverge_dat)) # remove non-convergence data
  ...
  #                     expected = sum(expected), nonconverged = sum(unlist(tmp$score_susie.converged) == FALSE))
  ...
  # for(met in methods){
  #   print(met)
  #   dat_sub = dat$fmv4 %>% filter(method == met) %>% filter(!(small_data.dataset %in% nonconverge_dat))
  ...
  #   for(met in methods){
  #     print(met)
  #     tmp = dat$susie %>% filter(method == met, sim_gaussian.n_signal == s)
  #     dat_sub = tmp %>% filter(!(small_data.dataset %in% nonconverge_dat)) # remove non-convergence data
  #     converged = unlist(dat_sub$score_susie.converged)
  #     converged[is.na(converged)] = FALSE
  #     total = unlist(dat_sub$score_susie.total) * converged
  ...
  output = paste0('susierss_ukb_20210324_REF500_pve005_cs/ukb_cs_remdatall', rem, '_refine')
  ...
  output = paste0('susierss_ukb_20210324_REF500_pve005_cs/ukb_cs_remdatall', rem, '_original')
  pdf(paste0(output, '_plots.pdf'), width=12, height=3)
  grid.arrange(p1,p2,p3,p4, ncol=4, widths=c(3,3,3,3))
  dev.off()
  system(paste0("convert -flatten -density 120 ", paste0(output, '_plots.pdf'), " ", paste0(output, '_plots.png')))
